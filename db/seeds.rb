...
puts "What would you like to seed?"
puts "1 - Recreate Countries"
puts "2 - Assign Attributes to Countries"
puts "3 - Assign Languages to Countries"
puts "4 - Assign Photos to Countries from countries_photos.json"
puts "5 - Assign Photos from Unsplash.it"
puts "6 - Overwrite countries_photos.json"
answer = STDIN.gets.chomp.to_i

if [1, 2, 3, 4, 5, 6].include? answer
  if answer == 1
    puts "Recreating Countries"
    ...
    COUNTRIES = {
       ...
       "Asia" => [
          ...
          {country: "Kazakhstan", currency_name: "Tenge", capital: "Astana", code: "TSE"},
      ...
      end # Recreate Countries
  end

  if answer == 2 || answer == 1 ## Assign Attributes to Countries
    puts "Assigning Attributes to Countries"
    attributes = [
      {
  ...
  if answer == 3 || answer == 1 ## Assign Languages to Countries
  ...
  if answer == 4 || answer == 1 ## Assign Photos to Countries from countries_photos.json
    countries = Country.all

    puts "Assigning photos from countries_photos.json"
    photos_file = File.read('db/countries_photos.json')
    photos_json = JSON.parse(photos_file)
    countries.each do |country|
      if country.photo.url.nil?
        image_url = photos_json[country.name]
  ...
  if answer == 5 ## Assign Photos from Unsplash.it
    puts "Assigning photos from unsplash it"
    countries.each do |country|
      if country.photo.url.nil?
  ...
  if answer == 6 || answer == 1 ## Overwrite countries_photos.json
    puts "Overwriting countries_photos.json"
    countries = Country.all
    # Write photos json
    photos = {}
    countries.each do |country|
      unless country.photo.url.nil?
        name = country.name
        url = country.photo.url

        photos[name] = url
      end
    end

    countries_json = File.open("db/countries_photos.json","w")
    countries_json.write(photos.to_json)
